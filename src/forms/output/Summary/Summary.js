 ...
 *     time for a while?
 */
const Summary = function ({ client, openFeedback }) {
  ...
  if (resourceKeys.length <= 1) {
    return (
      <FormattedMessage
        { ...messages.noBenefitsChosen } />
    );
  }
  if (client.future.earned === client.current.earned) {
    return (
      <FormattedMessage
        { ...messages.noFutureChange } />
    );
  }
  ...
  const detailsNow = (
    <p>
      <FormattedMessage
        { ...messages.details.current }
        values={{
          currentEarnings: toMoneyStr(current.earned),
          totalBenefits:   round$(current.benefitsTotal),
          currentTotal:    round$(current.total),
        }} />
    </p>
  );
  ...
  const detailsFuture = (
    <p>
      <FormattedMessage
        { ...messages.details.future }
        values={{
          futureEarnings: toMoneyStr(future.earned),
          totalBenefits:  round$(future.benefitsTotal),
        }} />
    </p>
  );
  ...
  const benefitList = [];
  const numBenefits = current.benefits.length;
  for (let benefitIndex = 0; benefitIndex < numBenefits; benefitIndex++) {

    let cBenefit = current.benefits[ benefitIndex ],
        fBenefit = future.benefits[ benefitIndex ];

    benefitList.push(
      <li key = { cBenefit.label }>
        <FormattedMessage
          { ...messages.details.benefit }
          values={{
            benefit:              cBenefit.label,
            currentBenefitAmount: round$(cBenefit.amount),
            futureBenefitAmount:  round$(fBenefit.amount),
          }} />
      </li>
    );
  ...
  const posDiff    = round$(Math.abs(diff));
  
  const diffFactor = diff === 0 ?
    0 :
    diff < 0 ?
      -1 :
      1;
  ...
  const summaryFuture = (
    <p>
      <FormattedMessage
        { ...messages.details.futureSummary }
        values={{
          diffFactor,
          change: posDiff,
          total:  round$(future.total),
        }} />
    </p>
  );

  let endOfCliffContent = (
    <FormattedMessage
      { ...messages.noCliff } />
  );
    ...
    endOfCliffContent = (
      <div>
        <div className="text-result-section">
          <Header>
            <FormattedMessage
              { ...messages.cliffEndHeader } />
          </Header>
          <p>
            <FormattedMessage
              { ...messages.cliffEndSummary } />
          </p>
        </div>
